...
/* *** LAST REVISED ON  7-JAN-1994 12:07:13.85 */
/* *** SOURCE FILE: [LONGLIB93.SOURCES.C.MASTER]PLOTLG2.C */
...
/* Subroutine */ int plotlg2_(x, y, nn, np, iflag, nsym, xaxl, yaxl, xtitl, 
...
/* 	REVISED BY D. LONG     FEB, 1986 -- ADDED LINE TYPE ARRAY */
...
/* 	ICOL	INT   ARRAY OF PEN COLORS DIMENSIONED ICOL(5+NN) */
/* 			ICOL(1) = GRID COLOR */
...
/* 			ICOL(8) = LINE COLOR 2 */
/* 			   .	     .	     3 */
/* 	ILINE	INT  LINE TYPE ARRAY FOR LINES DIMENSIONED ILINE(NN) */
...
/* Computing MAX */
	    r__1 = x[i];
	    cplotlg2_1.xm = dmax(r__1,cplotlg2_1.xm);
/* Computing MIN */
	    r__1 = x[i];
	    cplotlg2_1.dx = dmin(r__1,cplotlg2_1.dx);
/* L5: */
	}
	py[0] = cplotlg2_1.xm;
	py[1] = cplotlg2_1.dx;
	if (if_ == 2 || if_ == 3) {
	    r__1 = dabs(*xaxl);
	    scalg_(py, &r__1, &c__2, &c__1, &c__1, &cplotlg2_1.xm, &
		    cplotlg2_1.dx);
/* SMOOTH SCALE FACTORS */
	    ik = -1;
	    gdx = (float)1. / cplotlg2_1.dx;
	    nx = cplotlg2_1.dx * dabs(*xaxl);
	    inx = -1;
	} else {
	    r__1 = dabs(*xaxl);
	    scale_(py, &r__1, &c__2, &c__1, &c__1, &cplotlg2_1.xm, &
		    cplotlg2_1.dx);
/* SMOOTH SCALE FACTORS */
	    ik = 0;
	    gdx = (float)1.;
	...
	if (if_ == 2 || if_ == 3) {
	    cplotlg2_1.xm = dabs(*xmin) + (float)1e-25;
	    if (cplotlg2_1.xm != r_int(&cplotlg2_1.xm)) {
		cplotlg2_1.xm = r_int(&cplotlg2_1.xm) - (float)1.;
	    }
	    r__2 = dabs(*xmax) + (float)1e-25;
	    r__1 = r_lg10(&r__2);
	    cplotlg2_1.dx = r_int(&r__1) + (float)1.;
	    cplotlg2_1.dx = (cplotlg2_1.dx - cplotlg2_1.xm) / dabs(*xaxl);
	    gdx = (float)1. / cplotlg2_1.dx;
	    ik = -1;
	    nx = cplotlg2_1.dx * dabs(*xaxl);
	    inx = -1;
	    ...
	    cplotlg2_1.xm = *xmin;
	    cplotlg2_1.dx = (*xmax - *xmin) / dabs(*xaxl);
	    gdx = (float)1.;
	    ik = 0;
	    inx = 1;
	}
    ...
    if (*yaxl > (float)0.) {
/* INPUT SCALING */
	cplotlg2_1.ym = y[y_dim1 + 1];
	cplotlg2_1.dy = y[y_dim1 + 1];
	i__1 = *nn;
	for (in = 1; in <= i__1; ++in) {
	    i__2 = *np;
	    for (i = 1; i <= i__2; ++i) {
/* Computing MAX */
		r__1 = y[i + in * y_dim1];
		cplotlg2_1.ym = dmax(r__1,cplotlg2_1.ym);
/* Computing MIN */
		r__1 = y[i + in * y_dim1];
		cplotlg2_1.dy = dmin(r__1,cplotlg2_1.dy);
/* L6: */
	    }
	}
	py[0] = cplotlg2_1.ym;
	...
	if (if_ == 1 || if_ == 3) {
	    r__1 = dabs(*yaxl);
	    scalg_(py, &r__1, &c__2, &c__1, &c__1, &cplotlg2_1.ym, &
		    cplotlg2_1.dy);
/* SMOOTH SCALE FACTORS */
	    --ik;
	    gdy = (float)1. / cplotlg2_1.dy;
	    ny = cplotlg2_1.dy * dabs(*yaxl);
	    iny = -1;
	} else {
	    r__1 = dabs(*yaxl);
	    scale_(py, &r__1, &c__2, &c__1, &c__1, &cplotlg2_1.ym, &
		    cplotlg2_1.dy);
/* SMOOTH SCALE FACTORS */
	    ++ik;
	    gdy = (float)1.;
	...
	if (if_ == 1 || if_ == 3) {
	    ...
	    cplotlg2_1.ym = r_lg10(&r__1);
	    if (cplotlg2_1.ym != r_int(&cplotlg2_1.ym)) {
		cplotlg2_1.ym = r_int(&cplotlg2_1.ym) - (float)1.;
	    }
	    r__2 = dabs(*ymax) + (float)1e-25;
	    r__1 = r_lg10(&r__2);
	    cplotlg2_1.dy = r_int(&r__1) + (float)1.;
	    cplotlg2_1.dy = (cplotlg2_1.dy - cplotlg2_1.ym) / dabs(*yaxl);
	    gdy = (float)1. / cplotlg2_1.dy;
	    ny = cplotlg2_1.dy * dabs(*yaxl);
	    --ik;
	    iny = -1;
	    ...
	    cplotlg2_1.ym = *ymin;
	    cplotlg2_1.dy = (*ymax - *ymin) / dabs(*yaxl);
	    ++ik;
	    gdy = (float)1.;
	    iny = 1;
	}
...
/* AXIS LENGTH WITH TICK INFOR */
	...
	if (if_ == 2 || if_ == 3) {
	    i__2 = -ix - nadd;
	    lgaxs_(&c_b22, &c_b22, xtitl, &i__2, &xl, &c_b22, &cplotlg2_1.xm, 
		    &cplotlg2_1.dx, ic);
/* X-AXIS */
	    if (jf == 10) {
		r__1 = dabs(*yaxl);
		...
		lgaxs_(&c_b22, &r__1, xtitl, &i__2, &xl, &c_b22, &
			cplotlg2_1.xm, &cplotlg2_1.dx, ic);
	    }
/* X-AXIS */
	} else {
	    i__2 = -ix - nadd;
	    axis_(&c_b22, &c_b22, xtitl, &i__2, &xl, &c_b22, &cplotlg2_1.xm, &
		    cplotlg2_1.dx, &n1, &n2, ic);
/* X-AXIS */
	    if (jf == 10) {
		r__1 = dabs(*yaxl);
		i__2 = ix + 100 + nadd;
		axis_(&c_b22, &r__1, xtitl, &i__2, &xl, &c_b22, &
			cplotlg2_1.xm, &cplotlg2_1.dx, &n1, &n2, ic);
	    }
/* X-AXIS */
	}
    }
	...
	if (if_ == 1 || if_ == 3) {
	    i__2 = iy + nadd;
	    lgaxs_(&c_b22, &c_b22, ytitl, &i__2, &yl, &c_b42, &cplotlg2_1.ym, 
		    &cplotlg2_1.dy, ic);
/* Y-AXIS */
	    if (jf == 10) {
		r__1 = dabs(*xaxl);
		...
		lgaxs_(&r__1, &c_b22, ytitl, &i__2, &yl, &c_b42, &
			cplotlg2_1.ym, &cplotlg2_1.dy, ic);
	    }
/* Y-AXIS */
	} else {
	    i__2 = iy + nadd;
	    axis_(&c_b22, &c_b22, ytitl, &i__2, &yl, &c_b42, &cplotlg2_1.ym, &
		    cplotlg2_1.dy, &n1, &n2, ic);
/* Y-AXIS */
	    if (jf == 10) {
		r__1 = dabs(*xaxl);
		i__2 = -iy - 100 - nadd;
		axis_(&r__1, &c_b22, ytitl, &i__2, &yl, &c_b42, &
			cplotlg2_1.ym, &cplotlg2_1.dy, &n1, &n2, ic);
	    }
/* Y-AXIS */
	}
    }
...
/* LINE TYPE */
	    ...
	    line_(&x[1], &y[in * y_dim1 + 1], np, &c__1, nsym, &i__1, &c__1, &
		    c__1, &cplotlg2_1.xm, &cplotlg2_1.dx, &cplotlg2_1.ym, &
		    cplotlg2_1.dy);
/* PLOT DATA */
/* L78: */
	}
...
/* LINE TYPE */
	    ...
	    lglin_(&x[1], &y[in * y_dim1 + 1], np, &c__1, nsym, &i__1, &ik, &
		    c__1, &c__1, &cplotlg2_1.xm, &cplotlg2_1.dx, &
		    cplotlg2_1.ym, &cplotlg2_1.dy);
/* PLOT DATA */
/* L79: */
	}
    }
