...
C
	...
	CALL NUMBER(999.,999.,.2,FLOAT(JZ),0.,0.0,-1)
	CALL SYMBOL(999.,999.,.2,',',0.,1,-1)
	CALL NUMBER(999.,999.,.2,FLOAT(JW),0.,0.0,-1)
	IF (IAX.NE.0) THEN
		WVAL=(JW-1)*(EN(W(IFORM))-ST(W(IFORM)))/NW1+ST(W(IFORM))
		ZVAL=(JZ-1)*(EN(Z(IFORM))-ST(Z(IFORM)))/NZ1+ST(Z(IFORM))
		CALL SYMBOL(999.,999.,.2,') (',0.,3,-1)
		CALL NUMBER(999.,999.,.2,ZVAL,0.,0.02,-1)
		CALL SYMBOL(999.,999.,.2,',',0.,1,-1)
		CALL NUMBER(999.,999.,.2,WVAL,0.,0.02,-1)
	ENDIF
	CALL SYMBOL(999.,999.,.2,')',0.,1,-1)
C
