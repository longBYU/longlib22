	PROGRAM PLOTTESTS
C *** LAST REVISED ON  5-JAN-1994 10:31:18.86
C *** SOURCE FILE: [LONGLIB93.SOURCES.FORTRAN.EXAMPLES]PLOTTESTS.FOR
C
C	WRITTEN BY: DGL
C
...
C	ONE OF EACH KIND OF GRID.  THE AUTO-SCALING OPTION IS USED MOST
C	OF THE TIME.
C
...
C
C	INITIALIZE THE COLOR ARRAY (COLOR ALWAYS
C	USED  FOR THESE TESTS)
C
	DO 1 I=1,255
		ICOL(I)=I
1	CONTINUE
...
C	NOTE:  	FOR THESE TESTS OPEN THE PLOT PACKAGE BEFORE THE
C		MASTER ROUTINE CALLS.  SET THE IFLAG VALUES TO NOT
C		REOPEN THE PLOT PACKAGE AND ALSO TO LEAVE IS OPEN AFTER THE
C		THE CALL TO THEM.  THIS ILLUSTRATES HOW MULTIPLE MASTER
...
C
C	INITIALLY PLOT A PAGE OF GRAPHICS OUTPUT EXAMPLES TO
...
!	CALL PLOT(5.0,0.0,0)
c
c       choose one of the following lines depending on if compiler interpretts
c       \ characters inside of quoted strings
c
c	CALL SYMS(-0.3,1.0,0.2,'\\4SYMS Font 4 Text',90.0,18,-1)  ! g77
	CALL SYMS(-0.3,1.0,0.2,'\4SYMS Font 4 Text',90.0,18,-1)   ! most others
C       
...
C
C	MAKE AN ARBITRARY REGION AND FILL IT IN USING SHADE, 
...
C
	...
	CALL NEWPAGE
C
C	FOR THE REMAINING MASTER ROUTINE EXAMPLES DISPLAY SEVERAL
...
C
	CALL NEWPEN(-1)
C
C	FILL SOME ARRAYS WITH DATA TO PLOT, PICK SOME NICE FUNCTIONS
...
C
	CALL SCATPL(X2,Y2,2,25,-10014,ICOL,.1,8.,5.,'X AXIS',-6,
     $		'Y AXIS',-6,
     $		'SCATPL',-6,D1,D2,D3,D4,ICOL)
C
C	LET'S ADD A ELLIPSE TO INDICATE THE GROUPINGS AND ALSO TO ILLUSTRATE
C	HOW TO PLOT AFTER A MASTER ROUTINE CALL.  USE THE PLOTTING
...
C
	CALL PHIST(X,30,'Histogram',-9,6.,5.,
     $		'PHIST Subtitle',14,'X TITLE',
     $		7,-1.,1.,10.,-10001,1,0.,.4,ICOL)
C
...
C
	CALL GLPLOT(X,Y2,2,25,2,15,-10064,3,7.,5.,6.0102,-.15,5.02,
     $		'X Axis',
...
C
		Y3(I,1)=SIN(I*.4)
		Y3(I,2)=Y3(I,1)-0.5*RAN1(IS)
		Y3(I,3)=Y3(I,1)+0.7*RAN1(IS)
30	CONTINUE
	XX(1)=0.
	XX(2)=1.4
C
...
C
	CALL SEISPL(X,Y2,XX,2,25,2,20,-10034,-6,-.1,0.,6.,4.,
     $		'X Title',7,
...
C
 ...
 10	RETURN
	END
c
c       a pretty stupid, but portable random number generator
c
	real function RAN1(is)
	integer*4 is
	is=mod(is+9898911,1121241)
	RAN1=abs(cos(amod(float(is),2*3.151692654)))
	return
	end
