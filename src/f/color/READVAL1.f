	REAL FUNCTION READVAL1(DEF,IEND)
C
...
C
	...
	ILEN=6
	IDECFLAG=4
	IF (DEF.NE.0.0) ILEN=int(ALOG10(ABS(DEF)))
	ILEN=ILEN+ISIGN(1,ILEN)
	IF (ILEN.LT.0.0) IDECFLAG=ABS(ILEN)+4
	IF (ILEN.LT.0.0) ILEN=1
	ILEN=ILEN+IDECFLAG+1
	IF (DEF.LT.0.0) ILEN=ILEN+1
	IF (ILEN.LT.12) THEN
		WRITE (FM1,401) ILEN,IDECFLAG
		WRITE (FM2,FM1) DEF
 401		FORMAT('('' ['',F',I2.2,'.',I2.2,',''] '')')
	ELSE
		WRITE (FM2,402) DEF
 402		FORMAT(' [',E12.6,'] ')
	ENDIF
c1	FORMAT(' [',F<ILEN>.<IDECFLAG>,'] ',$)
c2	FORMAT(' [',E12.6,'] ',$)
	WRITE (*,403) FM2
 403	FORMAT(A,$)
